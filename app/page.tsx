...
function ClipsPage({ filter, setFilter, filtered, onBack, onNotice, onEdit, onRender }: { filter: "all" | "hot" | "rendered"; setFilter: (v: "all" | "hot" | "rendered") => void; filtered: Clip[]; onBack: () => void; onNotice: (message: string) => void; onEdit: (c: Clip) => void; onRender: (c: Clip) => void }) {
  return <div className="page clips-page">
    <div className="project-heading"><div><button className="back-link" onClick={onBack}>← Projects</button><div className="title-line"><h1>Podcast Bisnis: Mulai dari Nol</h1><span>Complete</span></div><p>35:42 · Bahasa Indonesia · Diproses 2 jam lalu</p></div><button className="outline-button" aria-label="Menu project" onClick={() => onNotice("Menu project segera tersedia")}>•••</button></div>
