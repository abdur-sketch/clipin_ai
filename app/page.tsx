...
export default function Home() {
  ...
  useEffect(()=>{fetch("/api/account").then(response=>response.ok?response.json():null).then(data=>{if(data?.subscription?.plan==="pro")setPlan(data.subscription.status==="trialing"?"Pro trial":"Pro plan");if(data?.user?.name)setAccountName(data.user.name);if(data?.user?.email)setAccountEmail(data.user.email)}).catch(()=>{})},[]);
  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {view === "dashboard" && <Dashboard projects={projects} clips={clipItems} onUpload={() => setUploadOpen(true)} onStudio={() => clipItems[0] ? setEditor(clipItems[0]) : setToast("Buat atau buka project terlebih dahulu")} onClips={() => go("clips")} onOpenProject={openProject} onProjects={() => go("projects")} />}
...
function Dashboard({ projects, clips, onUpload, onStudio, onClips, onOpenProject, onProjects }: { projects:ProjectSummary[];clips:Clip[];onUpload: () => void; onStudio: () => void; onClips: () => void; onOpenProject:(id:string)=>void;onProjects: () => void }) {
  return <div className="page dashboard-page">
    <div className="hero-copy"><div><span className="eyebrow"><i /> CREATE YOUR MOMENT</span><h1>One video.<br /><em>Many moments worth sharing.</em></h1><p>Kliyu AI menemukan bagian terbaik dari video Anda, membuat subtitle, dan menyiapkannya menjadi short content siap publikasi.</p><div className="hero-actions"><button className="primary" onClick={onUpload}><WandSparkles /> Create with Kliyu AI</button><button onClick={onStudio}><Pencil /> Open Studio</button></div></div><div className="hero-stats"><div><strong>24</strong><span>Clips created</span></div><div><strong>3</strong><span>Hot clips</span></div><div><strong>8m</strong><span>Processed</span></div></div></div>
...
function ClipEditor({ clip, onClose, onSave }: { clip: Clip; onClose: () => void; onSave: (clip: Clip) => void }) {
  ...
  async function uploadLogo(file?: File) { if(!file)return; setLogoName(file.name); if(typeof clip.id!=="string")return; const response=await fetch(`/api/clips/${clip.id}/logo`,{method:"PUT",headers:{"content-type":file.type,"content-length":String(file.size)},body:file}); if(!response.ok)setLogoName(""); }
