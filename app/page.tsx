...
type Clip = {
  ...
  aspectRatio?: string; fontSize?: number; watermark?: boolean; logoName?: string;
};
  ...
  async function startUpload(source?: File | string) {
    ...
    try {
      ...
      if (detailResponse.ok) {
        ...
        setClipItems(detail.clips.map((item, index) => ({ id: String(item.id), score: Number(item.score), duration: Math.max(1, Math.round(Number(item.end_time) - Number(item.start_time))), title: String(item.title), hook: String(item.hook), caption: String(item.caption), status: String(item.status) as Clip["status"], accent: accents[index % accents.length], startTime: Number(item.start_time), endTime: Number(item.end_time), style: String(item.style || "bold"), faceTracking: Boolean(item.face_tracking), hookOverlay: Boolean(item.hook_overlay), aspectRatio: String(item.aspect_ratio || "9:16"), fontSize: Number(item.font_size || 48), watermark: Boolean(item.watermark), logoName: item.logo_key ? String(item.logo_key).split("/").pop() : undefined })));
      }
    ...
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="nav-list" aria-label="Navigasi utama">
          <button className={view === "dashboard" ? "active" : ""} onClick={() => go("dashboard")}><span><HomeIcon /></span>Home</button>
          <button onClick={() => setUploadOpen(true)}><span><WandSparkles /></span>Kliyu AI</button>
        ...
        <div className="sidebar-bottom">
          <div className="usage-card"><div className="usage-icon"><Sparkles /></div><strong>{usage.used} dari {usage.limit} menit</strong><span>Terpakai bulan ini</span><div className="usage-bar"><i style={{width:`${Math.min(100,Math.round(usage.used/Math.max(1,usage.limit)*100))}%`}} /></div><button onClick={() => setUpgradeOpen(true)}>Upgrade Plan <ArrowUpRight /></button></div>
          <button className={`settings-button ${view==="settings"?"active":""}`} onClick={() => go("settings")}><span><Settings /></span>Settings</button>
          <div className="profile-wrap"><div className="profile"><div className="avatar">{accountName.split(/\s+/).map((part)=>part[0]).join("").slice(0,2).toUpperCase()}</div><div><strong>{accountName}</strong><span>{plan}</span></div><button aria-label="Menu profil" onClick={() => setProfileOpen(!profileOpen)}><MoreHorizontal /></button></div>{profileOpen&&<div className="profile-menu"><button onClick={()=>{go("settings");setProfileOpen(false)}}>Account settings</button><button onClick={()=>{setUpgradeOpen(true);setProfileOpen(false)}}>Billing & plan</button><a href="/signout-with-chatgpt?return_to=/">Sign out</a></div>}</div>
        </div>
      </aside>
      ...
      {upgradeOpen&&<UpgradeModal plan={plan} onClose={()=>setUpgradeOpen(false)} onUpgraded={()=>{setPlan("Pro trial");setUsage(value=>({...value,limit:300}));setUpgradeOpen(false);setToast("Trial Pro 7 hari berhasil diaktifkan")}}/>}
      {toast && <div className="toast"><span><CheckCircle2 /></span>{toast}</div>}
    </main>
  );
}

function Dashboard({ projects, clips, usage, onUpload, onStudio, onClips, onOpenProject, onProjects }: { projects:ProjectSummary[];clips:Clip[];usage:number;onUpload: () => void; onStudio: () => void; onClips: () => void; onOpenProject:(id:string)=>void;onProjects: () => void }) {
  return <div className="page dashboard-page">
    <div className="hero-copy"><div><span className="eyebrow"><i /> CREATE YOUR MOMENT</span><h1>One video.<br /><em>Many moments worth sharing.</em></h1><p>Kliyu AI menemukan bagian terbaik dari video Anda, membuat subtitle, dan menyiapkannya menjadi short content siap publikasi.</p><div className="hero-actions"><button className="primary" onClick={onUpload}><WandSparkles /> Create with Kliyu AI</button><button onClick={onStudio}><Pencil /> Open Studio</button></div></div><div className="hero-stats"><div><strong>{clips.length}</strong><span>Clips created</span></div><div><strong>{clips.filter(item=>item.score>=85).length}</strong><span>Hot clips</span></div><div><strong>{usage}m</strong><span>Processed</span></div></div></div>
...
function TemplatesPage({ onUse }: { onUse: (style: string) => void }) {
  const [category, setCategory] = useState("For You");
  const categories = ["For You","Podcast","Talking Head","Business","Education","Motivation","Islamic","Gaming"];
  const templates = [
    ["Bold Podcast","bold","Podcast"],["Minimal White","clean","Talking Head"],["Yellow Hook","bold","Business"],
    ["Karaoke Pop","karaoke","Motivation"],["Clean Education","clean","Education"],["Islamic Minimal","clean","Islamic"],["Business Pro","bold","Business"],
  ];
  const visible = category === "For You" ? templates : templates.filter((item) => item[2] === category);
  return <div className="page templates-page"><div className="simple-heading"><div><span className="eyebrow"><i /> KLIYU TEMPLATES</span><h1>Start with a proven style.</h1><p>Pilih template, lalu sesuaikan hasilnya di Kliyu Studio.</p></div></div><div className="template-filters">{categories.map((item)=><button key={item} className={category===item?"active":""} onClick={()=>setCategory(item)}>{item}</button>)}</div>{visible.length ? <div className="template-grid">{visible.map(([name,style,type],index)=><article key={name} className={`template-card template-${index%4}`}><div className="template-preview"><span className={`template-caption ${style}`}>{name.toUpperCase()}</span><Play /></div><div><span><small>{type}</small><strong>{name}</strong></span><button onClick={()=>onUse(style)}>Use Template <ArrowRight /></button></div></article>)}</div> : <div className="clips-empty"><LayoutTemplate /><h3>Template segera hadir</h3><p>Kategori ini sedang disiapkan oleh tim KLIYU.</p><button onClick={()=>setCategory("For You")}>Lihat rekomendasi</button></div>}</div>;
}
...
function SettingsPage({notify,onUpgrade,plan,accountName,accountEmail}:{notify:(message:string)=>void;onUpgrade:()=>void;plan:string;accountName:string;accountEmail:string}){
...
function ClipEditor({ clip, onClose, onSave, onNotice }: { clip: Clip; onClose: () => void; onSave: (clip: Clip) => void; onNotice:(message:string)=>void }) {
