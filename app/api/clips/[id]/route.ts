...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await bindings.DB.prepare("UPDATE clips SET title=?,hook=?,caption=?,start_time=?,end_time=?,style=?,face_tracking=?,hook_overlay=?,captions_enabled=?,aspect_ratio=?,font_size=?,font_family=?,font_color=?,font_effect=?,watermark=?,status='ready',rendered_key=NULL,updated_at=? WHERE id=?")
    .bind(title,hook,String(body.caption ?? ""),start,end,String(body.style ?? "bold"),body.faceTracking ? 1 : 0,body.hookOverlay ? 1 : 0,body.captionsEnabled === false ? 0 : 1,ratio,fontSize,fontFamily,fontColor,fontEffect,body.watermark ? 1 : 0,Date.now(),id).run();
