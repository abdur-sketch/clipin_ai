...
export async function PUT(request:Request){
  ...
  const user=await currentUser();const size=Number(request.headers.get("content-length")||0);if(size>15*1024*1024)return Response.json({error:"Backup maksimum 15 MB"},{status:413});
  const data=await request.json() as Record<string,unknown>;if(Number(data.version)!==1||!Array.isArray(data.projects)||!Array.isArray(data.clips))return Response.json({error:"Format backup KLIYU tidak valid"},{status:400});
  const originalProjects=data.projects as Record<string,unknown>[],originalClips=data.clips as Record<string,unknown>[];
  const projectMap=new Map(originalProjects.map((row)=>[String(row.id||""),`restore_prj_${crypto.randomUUID()}`]));
  const clipMap=new Map(originalClips.map((row)=>[String(row.id||""),`restore_clip_${crypto.randomUUID()}`]));
  ...
  const tables:Array<{name:string;rows:Record<string,unknown>[]}>= [{name:"projects",rows:projects},{name:"clips",rows:clips},{name:"transcripts",rows:transcripts},{name:"publications",rows:publications},{name:"revenue_entries",rows:revenue}];
  let restored=0;
  for(const table of tables){const schema=await bindings.DB.prepare(`PRAGMA table_info(${table.name})`).all<{name:string}>();const allowed=new Set(schema.results.map((column)=>column.name));const statements:D1PreparedStatement[]=[];for(const row of table.rows.slice(0,2000)){const columns=Object.keys(row).filter((key)=>allowed.has(key)&&/^[a-z_]+$/.test(key));if(!columns.length)continue;statements.push(bindings.DB.prepare(`INSERT INTO ${table.name} (${columns.join(",")}) VALUES (${columns.map(()=>"?").join(",")})`).bind(...columns.map((column)=>row[column]??null)));if(statements.length===80){await bindings.DB.batch(statements.splice(0));}restored++}if(statements.length)await bindings.DB.batch(statements)}
