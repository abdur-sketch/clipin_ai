...
export async function POST(request: Request) {
  ...
  let sourceUrl: string | null = null, sourceType = "upload";
  if (body.sourceUrl) {
    try {
      const parsed = new URL(body.sourceUrl.trim());
      if (!/^https?:$/.test(parsed.protocol)) return jsonError("Link video harus menggunakan http atau https");
      sourceUrl = parsed.toString();
      const host = parsed.hostname.toLowerCase();
      sourceType = host.includes("youtube.com") || host === "youtu.be" ? "youtube" : host.includes("tiktok.com") ? "tiktok" : host.includes("instagram.com") ? "instagram" : "url";
