...
export async function POST(request: Request) {
  const user = await currentUser(); const body = await request.json() as { title?: string; filename?: string; contentType?: string };
  if (!body.title?.trim()) return jsonError("Nama project wajib diisi");
  const projectId = id("prj"), now = Date.now();
  await bindings.DB.prepare("INSERT INTO projects (id,user_id,title,filename,content_type,status,progress,created_at,updated_at) VALUES (?,?,?,?,?,'draft',0,?,?)").bind(projectId,user.id,body.title.trim(),body.filename ?? null,body.contentType ?? null,now,now).run();
